...
def stochastic_quantization(v, s):
    ...
    实现随机量化 Q_s(v)
    参数：
        v: numpy array, 要量化的向量
        s: int, 量化等级（>=1）
    返回：
        qv: numpy array, 量化后的向量
    """
    ...
    abs_ratio = np.abs(v) / norm  # |v_i| / ||v||_2

    # 找到所在区间的 l
    l = np.floor(abs_ratio * s).astype(int)
    l = np.clip(l, 0, s - 1)  # 防止越界
...
if __name__ == '__main__':
    ...
    # qv_2 = [np.zeros(p) for i in range(n)]
    qv_1 = [[np.zeros(p) for j in range(m_i)] for i in range(n)]
    ...
    fh = logging.FileHandler(
        f'./log/z_logger_QML_E_{E}_rho_{rho}_m_{m}_n_{n}_m_i_{m_i}_v_max_{v_max}_eps_pri_{eps_pri}_eps_dual_{eps_dual}_alpha_{alpha}.log',
        'w')
    ...
    # 保存为 csv 文件
    df.to_csv(
        f'./csv/data_QML_E_{E}_rho_{rho}_m_{m}_n_{n}_m_i_{m_i}_v_max_{v_max}_iter_{N_iter}_eps_pri_{eps_pri}_eps_dual_{eps_dual}_alpha_{alpha}.csv',
        index=False)
