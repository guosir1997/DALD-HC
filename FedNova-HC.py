...
def analytical_solution_2_fog_i(w, x, y, i, mu_0, mu, rho):
    ...
    for j in range(m_i):
        grad_sum += p_ij[i][j] * alpha * s_ij[i][j]  # s_ij[i][j] 是向量

    # Step 2: x_i = x_i - tau_eff * grad_sum
    x_i_temp = x[i] - tau_eff[i] * grad_sum
...
def analytical_solution_2_cloud_center():
    w_temp = sum(p_i[i] * x[i] for i in range(n))
...
if __name__ == '__main__':
    ...
    s_ij = [[np.zeros(p) for j in range(m_i)] for i in range(n)]  # s_ij
    ...
    # τ^eff_i = Σ_j ( p_ij * τ_ij )
    tau_eff = np.sum(p_ij * E, axis=1)
    ...
    fh = logging.FileHandler(
        f'./log/z_logger_FedNova_E_{E}_rho_{rho}_m_{m}_n_{n}_m_i_{m_i}_v_max_{v_max}_eps_pri_{eps_pri}_eps_dual_{eps_dual}_alpha_{alpha}.log',
        'w')
    ...
    # 保存为 csv 文件
    df.to_csv(
        f'./csv/data_FedNova_E_{E}_rho_{rho}_m_{m}_n_{n}_m_i_{m_i}_v_max_{v_max}_iter_{N_iter}_eps_pri_{eps_pri}_eps_dual_{eps_dual}_alpha_{alpha}.csv',
        index=False)
